...
builder.Services.AddHttpClient<ITicketRepository, TicketRepository>(client =>
{
    ...
    if (!string.IsNullOrEmpty(cleanMistralKey))
    {
...
builder.Services.Configure<AskEiva.Domain.Services.JiraConfiguration>(builder.Configuration.GetSection("JiraConfiguration"));
...
// Multi-Source Hybrid Retrieval Repository
...
using (var scope = app.Services.CreateScope())
{
    try
    {
        // Safely invoke using the scoped dependency container pipeline
        var provisioner = scope.ServiceProvider.GetRequiredService<WeaviateSchemaProvisioner>();
        Console.WriteLine("[System Startup] Forcing validation schema check against your Weaviate instance...");
        await provisioner.EnsureSchemaAsync();
    }
    catch (Exception ex)
    {
        Console.WriteLine($"[Startup Provisioner Error Block]: {ex.Message}");
    }
}
